...
s32 SaveGameOnCard(u8 chan_par, u8 slot_par)
{
    ...
    if (event_res - 1U >= 2)
    {
        ...
        names_ptr = &PS1_SaveFilenames[-1][0]; // Unsure of the [-1][0] here, but it matches
        devname = names_ptr + (slot_par << 5);
        if (devname[0] == 0)
        {
            sprintf(devname, (char *) s_bu02xss4u_8012ade8, chan_par, s_BISLUS00005_8012aca8, save_ray[slot_par], (ushort) PS1_GlobalTimer);
        }
        ...
        if (fd != -1)
        {
            ...
            sprintf(card_frame0.Rayman_save_ray_pct, (char *) s_Rayman_s_03d_8012adf8, save_ray[slot_par], (short) (chan_local / 102));
